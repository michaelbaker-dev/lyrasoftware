...
type AiCallResult = {
  content: string;
  usedModel: string;
  provider: "openrouter" | "lmstudio" | "claude-code";
  promptTokens: number;
  completionTokens: number;
  totalTokens: number;
...
async function callClaudeCode(
  ...
  const result = await chatViaClaude(allMessages, model);

  return {
    content: result.content,
    usedModel: `${result.cliModel} (Claude Code)`,
...
async function callAI(
  ...
  if (model.startsWith("claude-code/")) {
    return callClaudeCode(systemPrompt, messages, model);
  }
...
async function withWebSearch(opts: {
  ...
  const isNonOpenRouter = opts.model.startsWith("local:") || opts.model.startsWith("claude-code/");

  if (isNonOpenRouter) {
    // Local and Claude Code models use Tavily for web search
  ...
  // OpenRouter models use :online suffix
  return {
...
The PRD should describe the current product state AND planned enhancements. Do not describe features that already exist as new requirements — instead reference them as existing functionality to build upon.`;
    ...
    const isClaudeCode = data.model.startsWith("claude-code/");
    ...
    const providerTag = isClaudeCode ? "Claude Code (Max)" : isLocal ? "LM Studio (local)" : "OpenRouter";
...
import { analyzeBreakdown, type BreakdownAnalysis } from "@/lib/breakdown-analyzer";
import type { TemplateConfig } from "@/lib/team-templates";
...
export async function setupLyraTeamInternal(data: {
  ...
  try {
    ...
    const { seedTemplates, applyTemplate, applyConfig } = await import("@/lib/team-templates");
    await seedTemplates();
    logs.push("Team templates seeded");

    // Check for saved team config (from the interactive team setup step)
    let templateSource: string;
    if (project.teamConfig) {
      const savedConfig: TemplateConfig = JSON.parse(project.teamConfig);
      logs.push("Using saved team configuration from onboarding");
      const result = await applyConfig(project.id, savedConfig);
      logs.push(...result.logs);
      logs.push("Applied saved team configuration successfully");
      templateSource = "custom";
    } else {
      // Fall back to template name (backward compat)
      templateSource = data.templateName || "Minimal";
      const result = await applyTemplate(project.id, templateSource);
      logs.push(...result.logs);
    }
    ...
    await prisma.auditLog.create({
      data: {
        ...
        actor: "system",
        details: JSON.stringify({ jiraKey: data.jiraKey, template: templateSource }),
      },
    });
...
export async function runValidationInternal(data: {
  ...
  try {
    ...
    // Safety net: if teamConfig exists but no teams were created (e.g. Team Setup step was skipped), apply it now
    const existingTeams = await prisma.team.findMany({ where: { projectId: project.id } });
    if (existingTeams.length === 0 && project.teamConfig) {
      logs.push("No teams found but teamConfig exists — applying saved team configuration...");
      try {
        const { applyConfig } = await import("@/lib/team-templates");
        const savedConfig: TemplateConfig = JSON.parse(project.teamConfig);
        const teamResult = await applyConfig(project.id, savedConfig);
        logs.push(...teamResult.logs);
        logs.push("Applied saved team configuration");
      } catch (e) {
        logs.push(`WARNING: Failed to apply saved team config: ${(e as Error).message}`);
      }
    }
...
// ── Team Recommendation & Config ────────────────────────────────────

export async function getTeamRecommendation(
  jiraKey: string
): Promise<{ success: boolean; analysis?: BreakdownAnalysis; error?: string }> {
  try {
    const project = await prisma.project.findUnique({
      where: { jiraKey },
      select: { breakdownContent: true },
    });

    if (!project?.breakdownContent) {
      return { success: false, error: "No breakdown found" };
    }

    const breakdown: WorkBreakdown = JSON.parse(project.breakdownContent);
    const analysis = analyzeBreakdown(breakdown);
...
export async function saveTeamConfig(
  ...
  try {
    // Validate: at least 1 team, each team has at least 1 agent
    if (!config.teams || config.teams.length === 0) {
      return { success: false, logs: [], error: "At least one team is required" };
    }
    for (const team of config.teams) {
      if (!team.agents || team.agents.length === 0) {
        return { success: false, logs: [], error: `Team "${team.name}" must have at least one agent` };
      }
    }

    await prisma.project.updateMany({
      where: { jiraKey },
      data: { teamConfig: JSON.stringify(config) },
    });

    return { success: true, logs: ["Team configuration saved"] };
  } catch (e) {
    return { success: false, logs: [], error: (e as Error).message };
  }
}

export async function getProjectTeamConfig(
  jiraKey: string
): Promise<{ teamConfig?: TemplateConfig; totalAgents: number; totalTeams: number }> {
  const project = await prisma.project.findUnique({
    where: { jiraKey },
    select: { teamConfig: true },
  });

  if (!project?.teamConfig) {
    return { totalAgents: 0, totalTeams: 0 };
  }

  const config: TemplateConfig = JSON.parse(project.teamConfig);
  const totalTeams = config.teams.length;
  const totalAgents = config.teams.reduce((sum, t) => sum + t.agents.length, 0);

  return { teamConfig: config, totalAgents, totalTeams };
}
